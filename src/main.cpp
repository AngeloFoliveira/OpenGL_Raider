...
int main(int argc, char *argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            speed = 50.0;
            camera_view_vector = glm::vec4(-cos(g_Phi) * sin(g_Theta), -sin(g_Phi), -cos(g_Phi) * cos(g_Theta), 0.0f); // Vetor "view", sentido para onde a câmera está virada
...
#define ALTURAMURO 20
        ...
        for (int i = 1; i <= 4; ++i)
        {
            ...
            if (i == 1)
            {
                ...
                model = Matrix_Translate(0.0f, 78.0f, 80.0f) * Matrix_Rotate_X(-3.14 / 2) * Matrix_Scale(80.0f, 20.0f, 80.0f);
                glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            }
            else if (i == 2)
            {
                // Desenhamos o plano da parede SUL
                model = Matrix_Translate(-80.0f, 78.0f, 0.0f) * Matrix_Rotate_Z(-3.14 / 2) * Matrix_Scale(80.0f, 20.0f, 80.0f);
                glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            }
            else if (i == 3)
            {
